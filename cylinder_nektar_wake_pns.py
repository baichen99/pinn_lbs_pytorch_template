...
from net import Net
from losses import cylinder_pns_loss, mse_loss
...
def train():
    tb = SummaryWriter('./tb_logs/cylinder_wake_pns')
    ...
    for epoch in range(20000):
        ...
        for obs_batch in obs_loader:
            obs_x = obs_batch[:, 0:1].requires_grad_()
            obs_y = obs_batch[:, 1:2].requires_grad_()
            obs_t = obs_batch[:, 2:3].requires_grad_()
        ...
        tb.add_scalar('loss/obs_loss', torch.mean(torch.stack(total_obs_loss)).item(), epoch)
        tb.add_scalar('loss/mse_pde_1', mse_pde_1.item(), epoch)
        tb.add_scalar('loss/mse_pde_2', mse_pde_2.item(), epoch)
        tb.add_scalar('loss/mse_pde_3', mse_pde_3.item(), epoch)
        tb.add_scalar('loss/mse_obs_1', mse_obs_1.item(), epoch)
        tb.add_scalar('loss/mse_obs_2', mse_obs_2.item(), epoch)

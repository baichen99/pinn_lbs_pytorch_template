...
def mse_loss(y, y_hat=None):
    if y_hat is None:
        return torch.mean(y ** 2)
    return torch.mean((y - y_hat) ** 2)

# pns pinn
def pns_pde_loss(x, y, t, u, v, p, nu_t):
...
def cylinder_ns_loss(x, y, t, u, v, p):
    ...
    continuity = u_x + v_y
    eq_1 = (u_t + (u*u_x + v * u_y) + p_x / rho - nu  * (u_xx + u_yy))
    eq_2 = (v_t + (u*v_x + v * v_y) + p_y / rho - nu  * (v_xx + v_yy))
    # mse for pde loss
